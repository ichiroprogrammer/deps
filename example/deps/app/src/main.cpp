...
public:
  ...
  virtual bool Output(std::ostream &) const noexcept override {
    return no_error_;
  }
...
// @@@ sample begin 0:0
std::unique_ptr<Dependency::ScenarioGenerator>
gen_scenario(App::DepsOpts const &opt) try {
  using namespace Dependency;

  switch (opt.GetCmd()) {
  case App::DepsOpts::Cmd::GenPkg:
    LOGGER("start GenPkg");
    return std::make_unique<PkgGenerator>(opt.In(), opt.IsRecursive(),
                                          opt.Dirs(), opt.Exclude());
  case App::DepsOpts::Cmd::GenSrc:
    LOGGER("start GenPkg");
    return std::make_unique<SrcsGenerator>(opt.In(), opt.IsRecursive(),
                                           opt.Dirs(), opt.Exclude());
  // @@@ ignore begin
  case App::DepsOpts::Cmd::GenPkg2Srcs:
    LOGGER("start GenPkg2Srcs");
    return std::make_unique<Pkg2SrcsGenerator>(
        opt.In(), opt.IsRecursive(), opt.IsSrcPkg(), opt.Dirs(), opt.Exclude());
  case App::DepsOpts::Cmd::GenPkg2Pkg:
    LOGGER("start GenPkg2Pkg");
    return std::make_unique<Pkg2PkgGenerator>(
        opt.In(), opt.IsRecursive(), opt.IsSrcPkg(), opt.Dirs(), opt.Exclude());
