...
std::string DepsOpts::Help() {
  ...
  ss << "         a    : generate structure to OUT from p2p output."
     << std::endl;
  ss << "         a2pu : generate plant uml package to OUT from p2p output."
     << std::endl;
  ...
  ss << "         --recursive : search dir as package from DIRS or IN contents."
     << std::endl;
  ss << "         -R          : same as --recursive." << std::endl;
  ss << "         --src_as_pkg: every src is as a package." << std::endl;
  ss << "         -s          : same as --src_as_pkg." << std::endl;
  ss << "         --log LOG   : logging to LOG(if LOG is \"-\", using STDOUT)."
     << std::endl;
  ss << "         --exclude PTN : exclude dirs which matchs to PTN(JS regex)."
     << std::endl;
...
DepsOpts::Cmd DepsOpts::parse_command(int argc, char *const *argv) {
  ...
  } else if (command == "h" || command == "help" || command == "-h" ||
             command == "--help") {
    return Cmd::Help;
  }
...
DepsOpts::DepsOptsData DepsOpts::parse(int argc, char *const *argv) {
  ...
  optind = 2;
  static struct option const opts[] = {{"in", required_argument, 0, 'i'},
                                       {"out", required_argument, 0, 'o'},
                                       {"exclude", required_argument, 0, 'e'},
                                       {"recursive", no_argument, 0, 'R'},
                                       {"src_as_pkg", no_argument, 0, 's'},
...
std::string ToStringDepsOpts(DepsOpts const &deps_opts,
                             std::string_view indent) {
  auto ss = std::ostringstream{};
  char const cmd[] = "cmd       : ";
  auto const indent2 =
      std::string(Nstd::ArrayLength(cmd) - 1, ' ') + std::string{indent};
  ...
  ss << indent << "dirs      : "
     << FileUtils::ToStringPaths(deps_opts.Dirs(), "\n" + indent2) << std::endl;
