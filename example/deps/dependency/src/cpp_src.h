...
class CppSrc {
public:
  explicit CppSrc(FileUtils::Path_t const &pathname,
                  FileUtils::Filename2Path_t const &db);
  FileUtils::Paths_t const &GetIncs() const noexcept { return incs_; }
  FileUtils::Paths_t const &GetIncsNotFound() const noexcept {
    return not_found_;
  }
...
private:
  ...
  friend bool operator==(CppSrc const &lhs,
                         CppSrc const &rhs) noexcept = default;
  friend auto operator<=>(CppSrc const &lhs,
                          CppSrc const &rhs) noexcept = default;
};
