...
FileUtils::Paths_t get_incs(FileUtils::Path_t const &src) {
  static auto const include_line =
      std::regex{R"(^\s*#include\s+["<]([\w/.]+)[">](.*))"};
  ...
  while (std::getline(f, line)) {
    ...
    if (auto results = std::smatch{};
        std::regex_match(line, results, include_line)) {
...
void get_incs_full(FileUtils::Filename2Path_t const &db,
                   FileUtils::Path_t const &src, FileUtils::Paths_t &incs,
  ...
  for (auto const &f : inc_files) {
    if (db.count(f) == 0) {
      ...
      if (!any_of(incs.cbegin(), incs.cend(),
                  [&full_path](FileUtils::Path_t const &p) noexcept {
                    return p == full_path;
                  })) {
...
CppSrc::CppSrc(FileUtils::Path_t const &pathname,
               FileUtils::Filename2Path_t const &db)
    : path_{FileUtils::NormalizeLexically(pathname)},
      filename_{path_.filename()}, incs_{}, not_found_{} {
  get_incs_full(db, pathname, incs_, not_found_, true);
}

CppSrcs_t GenCppSrc(FileUtils::Paths_t const &srcs,
                    FileUtils::Filename2Path_t const &db) {
  auto ret = CppSrcs_t{};
...
std::string ToStringCppSrc(CppSrc const &cpp_src) {
  ...
  ss << "file              : " << FileUtils::ToStringPath(cpp_src.Filename())
     << std::endl;
  ss << "path              : " << FileUtils::ToStringPath(cpp_src.Path())
     << std::endl;
  ss << "include           : "
     << FileUtils::ToStringPaths(cpp_src.GetIncs(), " ") << std::endl;
  ss << "include not found : "
     << FileUtils::ToStringPaths(cpp_src.GetIncsNotFound(), " ") << std::endl;

  return ss.str();
}

namespace {
constexpr std::string_view target_ext[]{".c",  ".h",   ".cpp", ".cxx",
                                        ".cc", ".hpp", ".hxx", ".tcc"};
...
FileUtils::Paths_t gen_dirs(FileUtils::Path_t const &top_dir,
                            FileUtils::Paths_t const &srcs) {
  auto dirs = FileUtils::Paths_t{top_dir};
  auto const top_dir2 =
      FileUtils::Path_t{""}; // top_dirが"."の場合、parent_path()は""になる}。
...
std::pair<FileUtils::Paths_t, FileUtils::Paths_t>
GetCppDirsSrcs(FileUtils::Paths_t const &dirs) {
