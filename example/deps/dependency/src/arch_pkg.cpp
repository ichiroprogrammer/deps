...
ArchPkg::Map_Path_ArchPkg_t
ArchPkg::build_depend_on(DepRelation const &dep_rel,
...
Arch_t ArchPkg::build_children(Map_Path_ArchPkg_t &&pkg_all) {
  ...
  for (auto &[path, pkg] : pkg_all) { // C++17 style
    ...
    if (pkg_all.count(parent_name) == 0) {
      ...
      ArchPkg *parent = cache.count(parent_name) != 0
                            ? cache.at(parent_name)
...
ArchPkg const *FindArchPkgByName(Arch_t const &arch,
                                 std::string_view pkg_name) noexcept {
  for (ArchPkgPtr_t const &pkg_ptr : arch) {
    if (pkg_ptr->Name() == pkg_name) {
      return pkg_ptr.get();
    } else {
      ArchPkg const *pkg_found =
          FindArchPkgByName(pkg_ptr->Children(), pkg_name);
      if (pkg_found) {
        return pkg_found;
      }
    }
...
ArchPkg const *FindArchPkgByFullName(Arch_t const &arch,
  ...
  for (ArchPkgPtr_t const &pkg_ptr : arch) {
    if (pkg_ptr->FullName() == full_name) {
      ...
      ArchPkg const *pkg_found =
          FindArchPkgByFullName(pkg_ptr->Children(), full_name);
      if (pkg_found) {
        return pkg_found;
      }
    }
...
std::string to_string_pkg(ArchPkg const &arch_pkg, uint32_t indent) {
  ...
  ss << indent_str << package << arch_pkg.Name() << cyclic_str(arch_pkg)
     << std::endl;
  ss << indent_str << full << arch_pkg.FullName() << std::endl;
  ss << indent_str << parent
     << (arch_pkg.Parent() ? arch_pkg.Parent()->Name() : top) << std::endl;
...
std::string to_pu_rectangle(ArchPkg const &pkg, uint32_t indent) {
  ...
  ss << indent_str << "rectangle \"" << pkg.Name() << "\" as "
     << unique_str_name(pkg.FullName());
...
// 単方向依存のみ
bool dep_is_cyclic(std::string const &from, std::string const &to,
...
std::string to_pu_rectangle(Arch_t const &arch, DepRelation const &dep_rel) {
  ...
  if (dep_rel.CountAtoB != 0) {
    ...
    if (dep_rel.CountBtoA != 0) {
      ...
      ss << "-[#" << pu_link_color(dep_rel.PackageA, dep_rel.PackageB, arch)
         << "]-> " << b;
    }
  } else if (dep_rel.CountBtoA != 0) {
    ss << b << " \"" << dep_rel.CountBtoA << "\" -[#"
       << pu_link_color(dep_rel.PackageB, dep_rel.PackageA, arch) << "]-> "
       << a;
  }
...
std::string ToPlantUML_Rectangle(Arch_t const &arch,
