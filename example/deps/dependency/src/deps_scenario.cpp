...
bool has_error_for_dir(FileUtils::Paths_t const &dirs) {
  ...
  if (not_dirs.size() != 0) {
    throw std::runtime_error{FileUtils::ToStringPaths(not_dirs) +
                             " not directory"};
  }
...
FileUtils::Paths_t remove_dirs_match_pattern(FileUtils::Paths_t &&dirs,
...
std::pair<FileUtils::Paths_t, FileUtils::Dirs2Srcs_t>
gen_dirs_and_dirs2srcs(FileUtils::Paths_t const &dirs, bool recursive,
                       std::string const &pattern) {
  auto ret =
      std::pair<FileUtils::Paths_t, FileUtils::Paths_t>{GetCppDirsSrcs(dirs)};
  ...
  auto dirs2srcs =
      FileUtils::Dirs2Srcs_t{FileUtils::AssginSrcsToDirs(dirs_pkg, srcs)};
...
FileUtils::Paths_t gen_dirs(std::string const &in, bool recursive,
                            FileUtils::Paths_t const &dirs_opt,
                            std::string const &pattern) {
  auto dirs = FileUtils::Paths_t{};

  if (in.size() != 0) {
    auto ret = std::optional<FileUtils::Paths_t>{
        FileUtils::LoadFromFile(in, Load_Paths)};
    if (!ret) {
...
bool includes(FileUtils::Paths_t const &dirs,
              FileUtils::Path_t const &dir) noexcept {
...
FileUtils::Dirs2Srcs_t
dirs2srcs_to_src2src(FileUtils::Paths_t const &dirs_opt,
...
FileUtils::Dirs2Srcs_t gen_dirs2srcs(std::string const &in, bool recursive,
                                     bool src_as_pkg,
                                     FileUtils::Paths_t const &dirs_opt,
                                     std::string const &pattern) {
  ...
  if (in.size() != 0) {
    ...
    auto ret =
        std::optional<FileUtils::Dirs2Srcs_t>{LoadFromFile(in, Load_Dirs2Srcs)};
    ...
    if (ret) {
      ...
      auto ret =
          std::optional<FileUtils::Paths_t>{LoadFromFile(in, Load_Paths)};
  ...
  return src_as_pkg ? dirs2srcs_to_src2src(dirs_opt, assign, recursive)
                    : assign;
}
...
bool SrcsGenerator::Output(std::ostream &os) const {
  auto ret =
      std::pair<FileUtils::Paths_t, FileUtils::Paths_t>{GetCppDirsSrcs(dirs_)};
...
Pkg2SrcsGenerator::Pkg2SrcsGenerator(std::string const &in, bool recursive,
                                     bool src_as_pkg,
                                     FileUtils::Paths_t const &dirs_opt,
                                     std::string const &pattern)
...
Pkg2PkgGenerator::Pkg2PkgGenerator(std::string const &in, bool recursive,
                                   bool src_as_pkg,
                                   FileUtils::Paths_t const &dirs_opt,
                                   std::string const &pattern)
    : dirs2srcs_{gen_dirs2srcs(in, recursive, src_as_pkg, dirs_opt, pattern)} {}
...
DepRels_t gen_dep_rel(std::string const &in) {
  ...
  auto ret =
      std::optional<DepRels_t>{FileUtils::LoadFromFile(in, Load_DepRels)};
...
struct ArchGenerator::Impl {
  Impl(DepRels_t &&a_dep_rels)
      : dep_rels(std::move(a_dep_rels)), arch(ArchPkg::GenArch(dep_rels)) {}
...
Arch2PUmlGenerator::Arch2PUmlGenerator(std::string const &in)
    : ArchGenerator{in} {}

bool Arch2PUmlGenerator::Output(std::ostream &os) const {
  os << "@startuml" << std::endl;
  os << "scale max 730 width"
     << std::endl; // これ以上大きいとpdfにした時に右端が切れる
...
CyclicGenerator::CyclicGenerator(std::string const &in)
    : ArchGenerator{in}, has_cyclic_dep_{
                             HasCyclicDeps(impl_->arch, impl_->dep_rels)} {}

bool CyclicGenerator::Output(std::ostream &os) const {
  os << "cyclic dependencies " << (has_cyclic_dep_ ? "" : "not ") << "found"
     << std::endl;
