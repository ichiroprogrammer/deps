...
struct DepRelation {
  explicit DepRelation(std::string package_a, uint32_t count_a2b,
                       FileUtils::Paths_t &&incs_a2b, std::string package_b,
                       uint32_t count_b2a, FileUtils::Paths_t &&incs_b2a)
      : PackageA{std::move(package_a)}, CountAtoB{count_a2b},
        IncsAtoB{std::move(incs_a2b)}, PackageB{std::move(package_b)},
        CountBtoA{count_b2a}, IncsBtoA{std::move(incs_b2a)} {}
  ...
  friend bool operator==(DepRelation const &lhs,
                         DepRelation const &rhs) noexcept = default;
};

inline auto operator<=>(DepRelation const &lhs,
                        DepRelation const &rhs) noexcept {
...
DepRels_t::const_iterator FindDepRels(DepRels_t const &dep_rels,
                                      std::string const &dirA,
