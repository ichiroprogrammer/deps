...
class CppDir {
public:
    explicit CppDir(FileUtils::Path_t const& path, CppSrcs_t&& srcs) : path_{path}, srcs_{std::move(srcs)} {}
...
inline std::ostream& operator<<(std::ostream& os, CppDir const& dir) { return os << ToStringCppDir(dir); }
}  // namespace Dependency
