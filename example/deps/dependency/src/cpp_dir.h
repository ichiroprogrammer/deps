...
public:
  ...
  // second 依存するヘッダファイル
  std::pair<uint32_t, FileUtils::Paths_t>
  DependsOn(CppDir const &cpp_pack) const;
...
private:
  ...
  friend bool operator==(CppDir const &lhs,
                         CppDir const &rhs) noexcept = default;
  friend auto operator<=>(CppDir const &lhs,
                          CppDir const &rhs) noexcept = default;
};

using CppDirs_t = std::vector<CppDir>;

CppDirs_t GenCppDirs(FileUtils::Paths_t const &srcs,
